...
void treat_state(struct request *req, int fd) {
    ...
    char game_path[FINISHEd_GAME_PATH_SIZE];
...
size_t create_temporary_state_file(struct request *req, int game_found, const char *fname) {
    ...
    /* Exctract word and hint from game file */
...
void move_to_SCORES(struct request *req, char code) {
    ...
    /* Check if a directory exists for req->PLID's completed games, otherwise
        create it */
    std::string completed_games = GAMES_DIR + req->PLID;
...
void create_game_session(std::string word, char moves, std::string PLID, std::string hint, struct request *req) {
    ...
    active_games.insert({PLID, new_game});
}
